...
- (void)testFunctionWithoutConstantArguments {
    ...
    XCTAssertEqualObjects(str, @"b,3.14,1,0,foo,bar", @"");
}

- (void)testBlockArgument {
    id block = block_test(^{ return 5; });
    XCTAssertFalse([block isKindOfClass:[NSValue class]], @"");

    int (^typedBlock)(void) = block;
    XCTAssertEqual(typedBlock(), 5, @"");
}
...
- (void)testMethodInvocation {
    ...
    XCTAssertEqual(count, (NSUInteger)4, @"");
}
